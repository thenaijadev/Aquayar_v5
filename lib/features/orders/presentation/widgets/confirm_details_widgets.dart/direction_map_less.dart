...
class _DirectionMapScreenLessState extends State<DirectionMapScreenLess> {
  ...
  void initState() {
    ...
    bloc.add(SocketEventConnectToHost(authToken: widget.data["token"]));
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<SocketBloc, SocketState>(
      listener: (context, state) {
        if (state is SocketStateOrderDispatched) {
          if (state.action is OrderAccepted) {
            Navigator.popAndPushNamed(context, Routes.payment);
          } else {
            InfoSnackBar.errorSnackBar(
                "Your order request was rejected by the driver");
            final OrderBloc bloc = context.read<OrderBloc>();
            bloc.add(OrderEventCancelOrder(
                token: widget.data["token"], orderId: widget.order.id));
          }
        }
      },
